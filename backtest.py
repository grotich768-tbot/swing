...
# Force UTF-8 for Windows console (rich tables)
...
PIP_USD_PER_LOT = {
    # Broker-verified: USD per pip per 1.0 standard lot (Eightcap)
    # Formula: tick_value × (pip_size / tick_size)
    "GOLD":   0.10,    # was 1.00  ✓ fixed
    "SILVER": 0.50,    # was 5.00  ✓ fixed
    "EURUSD": 10.00,    # was 10.00 ✓ fixed
    "GBPUSD": 10.00,    # was 10.00 ✓ fixed
    "USDJPY": 6.24,    # was 6.28  ✓ fixed
    "ETHUSD": 0.10,    # unchanged
    "BTCUSD": 0.10,    # unchanged
    "LTCUSD": 0.10,    # new
    "US30":   1.00,    # unchanged
    "US100":  1.00,    # unchanged
    "US500":  1.00,    # unchanged
    "UK100":  1.00,    # was 1.25  ✓ fixed
    "AUS200": 1.00,    # was 0.65  ✓ fixed
    "GER40":  1.00,    # was 1.10  ✓ fixed
    "JP225":  1.00,    # was 0.0065 ✓ fixed
}
...
# ─────────────────────────────────────────────────────────────────────────────
def fix_kaggle_unzipped_models(model_dir: Path):
    """
    Kaggle auto-extracts uploaded .zip files into directories.
    SB3 PPO.load requires actual .zip files. 
    This walks MODEL_DIR and re-zips any unzipped SB3 model directories.
    """
    import shutil
    if not model_dir.exists(): return
    for p in model_dir.iterdir():
        if p.is_dir() and (p / "data").exists() and (p / "pytorch_variables.pth").exists():
            zip_path = p.with_suffix(".zip")
            if not zip_path.exists():
                logger.info(f"Re-zipping Kaggle-extracted model: {p.name}")
...
def run_backtest(symbol: str, date_from: str, date_to: str,
    ...
    while not done:
        if _use_router:
            idx_for_feat = min(env._step_idx, len(feats) - 1)
            feat_row = feats[feat_cols].iloc[idx_for_feat].values
            info_dict = model.predict_verbose(obs, feat_row=feat_row)
            action = int(info_dict["action"])
            regime_name = info_dict.get("regime_name", "N/A")
        ...
        lots       = _lot_size(symbol, atr)
        step_pnl   = _pnl_usd(symbol, position, curr_price - prev_price, lots, flipped)
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    fix_kaggle_unzipped_models(MODEL_DIR)
    parser = argparse.ArgumentParser(
        description="Always-In Bot — Backtest with fixed position sizing."
    )
