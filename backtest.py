...
def _pnl_usd(symbol: str, position: int, price_change: float,
    ...
    if flipped:
        pnl -= SPREAD_PIPS[symbol] * pip_usd * lots   # spread cost
        pnl -= COMMISSION_USD[symbol] * lots            # commission
    return pnl
    ...
    for sym in args.symbols:
        try:
            all_metrics[sym] = run_backtest(
                sym, args.date_from, args.date_to, args.model
            )
