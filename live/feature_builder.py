...
class FeatureBuilder:
    ...
    def __init__(self, bridge: MT5Bridge, settings: LiveSettings):
        ...
        self._last_feat_row = {}  # symbol → most recent raw feature row (np.ndarray)
    ...
    def build_observation(
        ...
        try:
            ...
            # PRIMARY_TF=M15 needs ≥500 M15 bars (matches training _load_features)
            from config import PRIMARY_TF
            if PRIMARY_TF == "M15":
                if df_m15 is None or len(df_m15) < 500:
                    logger.error(
                        f"[{symbol}] Not enough M15 bars for feature engineering "
                        f"(have {0 if df_m15 is None else len(df_m15)}, need ≥500). "
                        f"Increase warmup_m15 in settings."
                    )
                    return None
            else:
            ...
            # transform_multi_tf auto-routes to M15-primary or H1-primary
            # based on config.PRIMARY_TF
            ...
            # Cache the most recent raw feature row so callers (e.g. the
            # live trader's RegimeRouter dispatch) can fetch it without
            # re-running feature engineering. Used for regime classification.
            self._last_feat_row[symbol] = (
                feats[feat_cols].iloc[-1].values.astype(np.float32)
            )
    ...
    def get_last_feat_row(self, symbol: str):
        """
        Return the most recent raw feature row for `symbol`, captured during
        the last successful build_observation() call.

        Used by RegimeRouter.predict(obs, feat_row=...) for regime
        classification — without this, RegimeRouter silently degrades to
        ensemble-only behaviour (feat_row defaults to None).

        Returns None if build_observation() hasn't been called for this
        symbol yet this session.
        """
        return self._last_feat_row.get(symbol)

    def get_last_atr(self, symbol: str) -> float:
        """
        Return the most recent ATR for position sizing.
        Defaults to PRIMARY_TF (M15 if configured, else H1).
        """
        from config import PRIMARY_TF
        return self.get_last_atr_tf(symbol, PRIMARY_TF)
