...
def _reentry_block_seconds() -> int:
    """Re-entry block duration = 1 bar of PRIMARY_TF."""
    from config import PRIMARY_TF
    return 900 if PRIMARY_TF == "M15" else 3600

_REENTRY_BLOCK_SECONDS = _reentry_block_seconds()   # block same-direction re-entry for 1 bar
...
def _bar_open_time(now: datetime) -> datetime:
    """
    Return the start time of the current bar, based on PRIMARY_TF.
    M15 → rounds down to the nearest :00/:15/:30/:45
    H1  → rounds down to the hour (legacy behaviour)
    """
    from config import PRIMARY_TF
    if PRIMARY_TF == "M15":
...
class LiveTrader:
    ...
    def _process_bar(self, symbol: str, now: datetime):
        ...
        if hasattr(model, "predict_verbose"):
            # RegimeRouter — MUST pass feat_row for regime classification.
            # Without it, feat_row defaults to None inside RegimeRouter.predict()
            # and the router silently falls through to ensemble-only behaviour,
            # never actually using its trained specialists.
            feat_row = self.builder.get_last_feat_row(symbol)
            if feat_row is None:
                logger.warning(
                    f"[{symbol}] RegimeRouter active but no feat_row cached — "
                    f"falling back to ensemble-only for this bar"
                )
    ...
    def _initialise_position(self, symbol: str, obs: np.ndarray):
        ...
        if model is None:
            return
        if hasattr(model, "predict_verbose"):
            # RegimeRouter — needs feat_row, same as the main _process_bar dispatch
            feat_row = self.builder.get_last_feat_row(symbol)
            action   = int(model.predict(obs, feat_row=feat_row))
        elif hasattr(model, "models"):
            action = int(model.predict(obs))
    ...
    def _calculate_lots(self, symbol: str) -> float:
        ...
        """
        try:
            # ATR sizing — PRIMARY_TF aware.
            # get_last_atr() returns PRIMARY_TF's ATR (M15 if configured).
            # We also check the next-larger TF for a "live spike" cross-check:
            #   M15 primary → cross-check vs H1 ATR/4 (1 H1 bar ≈ 4 M15 bars)
            #   H1  primary → cross-check vs M15 ATR×4
            from config import PRIMARY_TF
            atr_primary = self.builder.get_last_atr(symbol)   # PRIMARY_TF ATR

            if PRIMARY_TF == "M15":
                atr_h1_raw = self.builder.get_last_atr_tf(symbol, "H1")
                atr_h1_scaled = atr_h1_raw / 4.0   # H1 ATR → M15-equivalent
                atr = max(atr_primary, atr_h1_scaled)
                atr_h1, atr_m15_scaled = atr_h1_raw, atr  # for logging below
            else:
                atr_m15_raw = self.builder.get_last_atr_tf(symbol, "M15")
                atr_m15_scaled = atr_m15_raw * 4.0   # M15 ATR → H1-equivalent
                atr = max(atr_primary, atr_m15_scaled)
                atr_h1 = atr
            ...
            logger.debug(
                ...
                f"M15×4={atr_m15_scaled:.4f}  atr={atr:.4f}  lots={lots:.3f}"
            )
            return lots
        except Exception as e:
    ...
    def _load_models(self):
        """
        Load models per-symbol according to config.get_model_mode(symbol),
        which encodes the empirically-determined best mode from comparing
        ensemble vs regime-router backtests (see config.py SYMBOL_MODEL_MODE
        for the source numbers).

        Within the preferred mode, falls back gracefully if files are
        missing:
          mode="regime"   -> RegimeRouter -> EnsemblePredictor -> single PPO
          mode="ensemble" -> EnsemblePredictor -> single PPO
        This means a symbol explicitly configured for "ensemble" will NEVER
        silently load a RegimeRouter (avoiding the issue where, before this
        per-symbol config existed, every symbol always tried RegimeRouter
        first regardless of which mode actually performed better for it).
        """
        from stable_baselines3 import PPO
        from config import get_model_mode

        for sym in self.s.active_symbols:
            mode   = get_model_mode(sym)
            loaded = False

            logger.info(f"[{sym}] Preferred model mode: {mode}")
            ...
            if mode == "regime":
                try:
                    ...
                    logger.warning(
                        f"[{sym}] RegimeRouter unavailable ({e}) — "
                        f"falling back to ensemble"
                    )

            # Ensemble — either the preferred mode, or the fallback from a
            # missing/incomplete regime router
            if not loaded:
                try:
            ...
            if not loaded:
                ...
                actual_kind = (
                    "regime" if hasattr(self.models[sym], "predict_verbose") else
                    "ensemble" if hasattr(self.models[sym], "models") else
                    "single"
                )
                if mode == "regime" and actual_kind != "regime":
                    logger.warning(
                        f"[{sym}] Configured for regime mode but loaded "
                        f"'{actual_kind}' instead — check that regime agent "
                        f"files exist in {MODEL_DIR}"
                    )
