...
def _session_multiplier(hour: int, minute: int = 0) -> float:
    """
    Session-aware reward multiplier — M15 aware (uses hour + minute).
    ...
    Calibrated from GOLD ensemble backtest (2022-2026):
      ...
      Asia    00-07 UTC  $163/trade, 72.0% win  (strong for GOLD/metals)
      NY      16-22 UTC  $137/trade, 73.1% win
      Off     22-00 UTC  $121/trade, 70.5% win

    Power hours (15-min precision for M15 bars):
      09:00-09:45  London open — highest win rate
      13:00-13:45  NY pre-open
      05:00-05:45  Early Tokyo
      17:00-17:45  NY first hour
    """
    from config import SESSION_POWER_HOURS
    # Power-hour override: matches on hour only, applies to full hour's M15 bars
...
class AlwaysInEnv(gym.Env):
    ...
    def step(self, action: int):
        ...
        session_mult = _session_multiplier(hour, minute)
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        pos_vec   = np.array([
            ...
            _session_multiplier(hour, minute),     # session quality signal
    ...
    def _get_minute(self) -> int:
        """Return UTC minute for the current step (for M15 granularity)."""
        if self._timestamps is not None:
            try:
                ts = self._timestamps[self._step_idx]
