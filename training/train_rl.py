...
def _load_features(loader, engineer, symbol, date_from, date_to):
    """
    Load all TFs and return (feats_df, feat_cols, raw_primary).
    M15 mode: primary=M15, context=H1/H4/D1
    H1 mode:  primary=H1,  context=M15/H4/D1
    """
    ...
    if PRIMARY_TF == "M15":
        if raw_m15 is None or len(raw_m15) < 500:
    ...
    return feats, feat_cols, raw_primary
...
def _train_one_seed(
    ...
    # Check if final model already exists! If so, skip training entirely.
    final_path = MODEL_DIR / f"ppo_{symbol}_seed{seed}_final"
    if final_path.with_suffix(".zip").exists():
        logger.info(f"[{symbol}] seed={seed} — Final model already exists! Skipping training.")
