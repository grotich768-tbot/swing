...
class FeatureEngineer:
    ...
    def transform_multi_tf(
        ...
        M15 mode (PRIMARY_TF = "M15"):
          M15  →  PRIMARY features (bars aligned to M15)
          H1   →  session context (most recent H1 before each M15 bar)
          H4   →  regime gate + setup context
          D1   →  daily bias

        H1 mode (PRIMARY_TF = "H1") — legacy:
          H1   →  primary features
        ...
        if PRIMARY_TF == "M15" and df_m15 is not None and len(df_m15) >= 500:
            return self._transform_m15_primary(
                df_m15, df_h1, df_h4, df_d1, symbol
            )

        # ── H1 primary (legacy / fallback) ────────────────────────────────────
        ...
        logger.debug(
            f"[{symbol}] Multi-TF (H1 primary): {loaded}  "
    ...
    def _transform_m15_primary(
        ...
        """
        M15 as primary timeframe.
        All features aligned to M15 bars (~1.5hr avg hold).
        H1/H4/D1 used as context layers only.
        """
        # ── M15 primary features ──────────────────────────────────────────────
        m15_feats = pd.DataFrame(index=df_m15.index)
        m15_feats = self._h1_features(m15_feats, df_m15)   # same features, M15 bars
        m15_feats["_close"] = df_m15["close"]
        m15_feats["_atr"]   = self._true_range(df_m15).rolling(ATR_PERIOD).mean()
        m15_feats.dropna(inplace=True)
        m15_feats["_target_direction"] = self._direction_label(df_m15, DIRECTION_HORIZON)
        ...
        if df_h1 is not None and len(df_h1) >= 50:
            h1_ctx = self._build_h1_context(df_h1)
            h1_aln = h1_ctx.reindex(m15_feats.index, method="ffill")
            parts.append(h1_aln)
            loaded.append("H1")
        ...
        if df_h4 is not None and len(df_h4) >= 50:
            h4_ctx = self._build_h4_context(df_h4)
            h4_aln = h4_ctx.reindex(m15_feats.index, method="ffill")
            parts.append(h4_aln)
            loaded.append("H4")
        ...
        if df_d1 is not None and len(df_d1) >= 50:
            d1_ctx = self._build_d1_context(df_d1)
            d1_aln = d1_ctx.reindex(m15_feats.index, method="ffill")
            parts.append(d1_aln)
            loaded.append("D1")
        ...
        n_feat = len([c for c in combined.columns
                      if not c.startswith("_") and not c.startswith("target")])
        logger.debug(
            f"[{symbol}] Multi-TF (M15 primary): {loaded}  "
            f"→ {n_feat} features  {len(combined):,} bars"
        )
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # H1 session context when M15 is primary  (8 features, prefix h1c_)
    # ─────────────────────────────────────────────────────────────────────────
    def _build_h1_context(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        H1 context features used when M15 is the primary timeframe.
        Gives the agent the current H1 trend/momentum without running
        the full H1 feature set twice.
        """
        ctx   = pd.DataFrame(index=df.index)
        close = df["close"]

        ctx["h1c_ret_1"] = np.log(close / close.shift(1))
        ...
        rs    = (up.ewm(com=13, min_periods=14).mean() /
                 dn.ewm(com=13, min_periods=14).mean())
        ctx["h1c_rsi"] = (100 - 100 / (1 + rs) - 50) / 50

        tr  = self._true_range(df)
        atr = tr.rolling(14).mean()
        ctx["h1c_atr_pct"] = atr / (close + 1e-10)

        ema8  = close.ewm(span=8,  adjust=False).mean()
        ema21 = close.ewm(span=21, adjust=False).mean()
        ema50 = close.ewm(span=50, adjust=False).mean()
        ctx["h1c_ema8_rat"]  = close / (ema8  + 1e-10) - 1
        ctx["h1c_ema21_rat"] = close / (ema21 + 1e-10) - 1
        ctx["h1c_trend_up"]  = (
        ...
        macd   = ema12 - ema26
        signal = macd.ewm(span=9, adjust=False).mean()
        ctx["h1c_macd_hist"] = (macd - signal) / (close + 1e-10)
